...
BULLET_PATTERN = [[
    r"[\s\u3000]*第[零一二三四五六七八九十百0-9]+(分?编|部分)",
    r"[\s\u3000]*第[零一二三四五六七八九十百0-9]+章",
    r"[\s\u3000]*第[零一二三四五六七八九十百0-9]+节",
    r"[\s\u3000]*第[零一二三四五六七八九十百0-9]+条",
    r"[\(（][零一二三四五六七八九十百]+[\)）]",
], [
    r"第[0-9]+章",
    r"第[0-9]+节",
...
class Node:
    def __init__(self, level, depth=-1, texts=None, law_name=None):
        self.level = level
        self.depth = depth
        self.texts = texts or []
        self.children = []
        self.law_name = law_name  # 法律名称 
    ...
    def build_tree(self, lines):
        ...
        for level, text in lines:
            ...
            # 创建子节点时，继承法律名称
            node = Node(level=level, texts=[text], law_name=self.law_name)
    ...
    def get_tree(self):
        tree_list = []  
        # 从根节点获取法律名称
        root_law_name = self.law_name if hasattr(self, 'law_name') else None
        self._dfs(self, tree_list, [], root_law_name)
        return tree_list

    def _dfs(self, node, tree_list, titles, law_name=None):
        ...
        # 关键修正：level == depth的节点（即"条"）也应该作为独立块，而不仅仅是作为标题
        if 1 <= level < self.depth:
            # 层级小于depth：作为标题路径累积
            path_titles = titles + texts
        elif level == self.depth:
            # 层级等于depth（即"条"）：作为标题路径，但如果有texts，会在下面生成独立块
            path_titles = titles + texts
        else:
            # 层级大于depth：不累积到路径
            path_titles = titles

        # level == depth：直接生成包含标题路径的完整块
        if level == self.depth and texts:
            # 如果有法律名称，在标题路径前加上法律名称
            if law_name:
                final_path = [law_name] + path_titles
            else:
                final_path = path_titles
            tree_list.append("\n".join(final_path))
            return
        # 没有子节点且层级仍在depth之内：将标题路径作为独立块
        if not child and (1 <= level < self.depth):
            # 如果有法律名称，在标题路径前加上法律名称
            if law_name:
                final_path = [law_name] + path_titles
            else:
        ...
        # 递归遍历子节点，仅当层级仍在depth范围内
        if level < self.depth:
            for c in child:
                self._dfs(c, tree_list, path_titles, law_name)