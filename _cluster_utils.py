...
try:
    ...
    try:
        ...
        try:
            ...
            print("Warning: umap-learn not available, using PCA for dimensionality reduction")
...
def convert_response_to_json(response: str) -> dict:
    ...
    if not prediction_json:
        ...
        logger.error(f"Raw response: {response[:200]}...")  # 记录前200个字符用于调试
        # 返回包含必要字段的默认字典，避免KeyError
        return {
            'entity_name': 'Unknown_Community',
            'entity_description': 'Failed to generate community description',
    ...
    if 'entity_name' not in prediction_json:
        prediction_json['entity_name'] = 'Unknown_Community'
        logger.warning("entity_name field missing in LLM response, using default value")
    
    if 'entity_description' not in prediction_json:
        prediction_json['entity_description'] = 'Failed to generate community description'
        logger.warning("entity_description field missing in LLM response, using default value")
    
    if 'findings' not in prediction_json:
        prediction_json['findings'] = "No specific findings available"
        logger.warning("findings field missing in LLM response, using default value")
    ...
    if UMAP_AVAILABLE:
        try:
            ...
            reduced_embeddings = UMAP(
                    n_neighbors=n_neighbors, n_components=dim, metric=metric, verbose=False, low_memory=False
            ).fit_transform(embeddings)
        except Exception as e:
            # UMAP失败时回退到PCA
            logger.warning(f"UMAP failed with error: {e}, falling back to PCA")
            print(f"Using PCA instead of UMAP for dimensionality reduction due to error: {type(e).__name__}")
            reduced_embeddings = PCA(n_components=dim).fit_transform(embeddings)
    else:
        # 使用PCA作为替代
        print("Using PCA instead of UMAP for dimensionality reduction")
        reduced_embeddings = PCA(n_components=dim).fit_transform(embeddings)
...
def local_cluster_embeddings(
    ...
    if UMAP_AVAILABLE:
        try:
            reduced_embeddings = UMAP(
                    n_neighbors=num_neighbors, n_components=dim, metric=metric, verbose=False, low_memory=False
            ).fit_transform(embeddings)
        except Exception as e:
            # UMAP失败时回退到PCA
            logger.warning(f"UMAP failed with error: {e}, falling back to PCA")
            print(f"Using PCA instead of UMAP for local dimensionality reduction due to error: {type(e).__name__}")
            reduced_embeddings = PCA(n_components=dim).fit_transform(embeddings)
    else:
        # 使用PCA作为替代
        print("Using PCA instead of UMAP for local dimensionality reduction")
        reduced_embeddings = PCA(n_components=dim).fit_transform(embeddings)
    return reduced_embeddings
...
def process_cluster( 
    ...
    # 检查必要字段是否存在，如果不存在则提供默认值
    if not data:
        logger.error("Failed to extract JSON data from LLM response, using fallback")
        data = {}
    
    # 确保必要字段存在
    if 'entity_name' not in data:
        # 使用第一个子节点的名称作为默认名称
        data['entity_name'] = f"cluster_{layer}_{len(cluster_nodes)}"
        logger.warning(f"entity_name missing in LLM response, using default: {data['entity_name']}")
    
    if 'entity_description' not in data:
        # 使用所有子节点描述的组合作为默认描述
        descriptions = [node.get('description', '') for node in cluster_nodes if 'description' in node]
        data['entity_description'] = " | ".join(descriptions[:3])  # 只取前3个描述避免过长
        logger.warning("entity_description missing in LLM response, using combined descriptions")
...
def process_relation( 
    ...
    **get_direct_relations(cluster1_nodes,cluster2_nodes,generate_relations)}
    ...
    # 安全地获取findings字段，如果不存在则使用默认值
    cluster1_data = community_report[maybe_edge[0]]
    cluster2_data = community_report[maybe_edge[1]]
    
    cluster1_description = cluster1_data.get('findings', cluster1_data.get('entity_description', 'No description available'))
    cluster2_description = cluster2_data.get('findings', cluster2_data.get('entity_description', 'No description available'))
    
    # 如果仍然没有描述，使用children信息作为描述
    if cluster1_description == 'No description available':
        cluster1_description = f"Cluster containing: {', '.join(cluster1_data.get('children', [])[:3])}"
    if cluster2_description == 'No description available':
        cluster2_description = f"Cluster containing: {', '.join(cluster2_data.get('children', [])[:3])}"
    ...
    if tokens>allowed_tokens:
        ...
        # 检查并截断过长的 relation_information，避免超过模型最大长度
        # 预留空间给 prompt 模板和其他内容（约 2000 tokens）
        max_relation_tokens = 14000  # 假设 max_model_len 是 16384，预留空间
        relation_info_text = "\n".join(relation_infromation)
        relation_tokens = len(tokenizer.encode(relation_info_text))
        
        if relation_tokens > max_relation_tokens:
            print(f"⚠️  警告: relation_information 长度 ({relation_tokens} tokens) 超过限制 ({max_relation_tokens} tokens)，进行截断")
            # 截断 relation_information，保留最重要的关系
            # 优先保留较长的描述（可能包含更多信息）
            relation_infromation_sorted = sorted(relation_infromation, key=len, reverse=True)
            truncated_relations = []
            current_tokens = 0
            
            for rel in relation_infromation_sorted:
                rel_tokens = len(tokenizer.encode(rel))
                if current_tokens + rel_tokens <= max_relation_tokens:
                    truncated_relations.append(rel)
                    current_tokens += rel_tokens
                else:
                    break
            
            # 如果还有空间，添加一些较短的关系
            if current_tokens < max_relation_tokens:
                for rel in relation_infromation:
                    if rel not in truncated_relations:
                        rel_tokens = len(tokenizer.encode(rel))
                        if current_tokens + rel_tokens <= max_relation_tokens:
                            truncated_relations.append(rel)
                            current_tokens += rel_tokens
            ...
            relation_info_text = "\n".join(truncated_relations)
            print(f"   截断后长度: {len(tokenizer.encode(relation_info_text))} tokens，保留了 {len(truncated_relations)}/{len(relation_infromation)} 个关系")
        ...
        exact_prompt=cluster_cluster_relation_prompt.format(entity_a=maybe_edge[0],entity_b=maybe_edge[1],\
            entity_a_description=cluster1_description,entity_b_description=cluster2_description,\
                relation_information=relation_info_text,tokens=gene_tokens)
        
        # 最终检查 prompt 长度
        prompt_tokens = len(tokenizer.encode(exact_prompt))
        if prompt_tokens > 16384:
            print(f"⚠️  警告: 最终 prompt 长度 ({prompt_tokens} tokens) 仍然超过限制，进一步截断")
            # 进一步截断 relation_information
            max_relation_tokens = 12000
            relation_info_text = "\n".join(relation_infromation[:len(relation_infromation)//2])  # 只保留一半
            exact_prompt=cluster_cluster_relation_prompt.format(entity_a=maybe_edge[0],entity_b=maybe_edge[1],\
                entity_a_description=cluster1_description,entity_b_description=cluster2_description,\
                    relation_information=relation_info_text,tokens=gene_tokens)
            prompt_tokens = len(tokenizer.encode(exact_prompt))
            print(f"   最终 prompt 长度: {prompt_tokens} tokens")
...
class Hierarchical_Clustering(ClusteringAlgorithm):
    def perform_clustering(
        ...
        community_report_prompt = PROMPTS.get("aggregate_entities_zh", PROMPTS.get("aggregate_entities", ""))
        cluster_cluster_relation_prompt = PROMPTS.get("cluster_cluster_relation_zh", PROMPTS.get("cluster_cluster_relation", ""))
