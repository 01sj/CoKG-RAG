...
修改历史：
- 2024-12-12: 使用本地模型路径 /newdatad/WHH/MyEmoHH/models/Qwen2-7B-Instruct/
- 配置：GPU 2 和 GPU 3（双卡并行），Qwen2-7B-Instruct（tensor_parallel_size=2）
- 备注：
  - DeepSeek-V2-Lite-Chat: MoE 模型，需要 23GB+ 显存，单卡无法加载
  - Qwen2-7B-Instruct: 需要 14-16GB 显存，单卡会出现 KV cache 内存不足
  - Qwen2-7B-Instruct（双卡）: 使用双卡并行，每张卡约 7-8GB 显存，可正常运行
  - 改用 GPU 2 和 GPU 3（第三和第四张卡），避免与其他进程冲突
"""
...
def setup_vllm_direct(config):
    ...
    gpu_ids = config.get('gpu_ids', '3')  # 默认使用 GPU 3（第四张卡）
    ...
    if current_cuda_devices:
        ...
        print(f"当前使用的 GPU: {current_cuda_devices} (物理 GPU 编号，vLLM 会将其视为 GPU 0)")
    else:
    ...
    # 限制并发数量，默认2（可根据GPU数量和内存调整）
    # 如果遇到OOM错误，可以降低到1；如果GPU利用率不高，可以提高到3-4
    # 建议值：tensor_parallel_size=1 时，并发数设为 1-2 较合适
    max_concurrent = int(os.environ.get('VLLM_MAX_CONCURRENT', '1'))
...
def main():
    ...
    # 使用社会法数据的绝对路径
    chunk_input = "/newdataf/SJ/LeanRAG/datasets/chunks/basic_laws_social_only_chunk.json"
    output_dir = "/newdataf/SJ/LeanRAG/output/social_law_7B"
    ...
    if os.environ.get('VLLM_MODEL_PATH'):
        ...
        # 默认使用本地 Qwen2-7B-Instruct 模型（约 14-16GB 显存，双卡并行每卡约 7-8GB）
        ...
        print(f"   配置: 使用 tensor_parallel_size=2 双卡并行，每张卡约 7-8GB 显存")
    
    # Qwen2-7B-Instruct 模型配置（单卡模式）
    # 该模型约需 14-16GB 显存，使用单卡时需要较高的内存利用率
    # 单卡模式需要调整内存配置以适应24GB显存
